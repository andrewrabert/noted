...
impl OAuthProvider {
    ...
    fn park_txn(&self, txn: String, query: HashMap<String, String>) -> Result<()> {
        ...
        if txns.len() >= MAX_TXNS {
            return Err(noted::error::unavailable(
...
fn provider(state: &AuthState) -> &OAuthProvider {
...
async fn login_post(State(state): State<AuthState>, headers: HeaderMap, body: Bytes) -> Response {
    let p = provider(&state);
