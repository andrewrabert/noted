...
fn an_empty_env_file_var_falls_back_to_the_config_dir() {
    let file = EnvFile::resolve(Some(Path::new("")), Some(Path::new("/home/u/.config"))).unwrap();
    assert_eq!(file.path(), Path::new("/home/u/.config/noted.env"));
}

#[test]
fn no_config_dir_and_no_override_means_no_env_file() {
    assert!(EnvFile::resolve(None, None).is_none());
}

#[test]
fn a_malformed_env_file_is_rejected() {
    let file = EnvFile::resolve(Some(Path::new("/etc/noted.env")), None).unwrap();
    let err = file
