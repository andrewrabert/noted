...
#[tokio::test]
async fn an_attachment_turns_a_plain_task_into_a_task_directory() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "t", "", "").await.unwrap();
    assert!(task_file(&dir, "task_0001").is_file());

    let at = attach(&root, "task_0001", "hello.txt", HELLO)
        .await
        .unwrap();
...
#[tokio::test]
async fn a_task_keeps_its_reference_and_markdown_across_an_attachment() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "keep me", "dev", "the body\n").await.unwrap();
    let before = std::fs::read(task_file(&dir, "dev/task_0001")).unwrap();

    attach(&root, "dev/task_0001", "hello.txt", HELLO)
        .await
        .unwrap();
    ...
    let after = get(&root, "dev/task_0001", false).await.unwrap();
...
#[tokio::test]
async fn a_second_attachment_lands_beside_the_first() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "t", "", "").await.unwrap();
    attach(&root, "task_0001", "one.txt", ONE).await.unwrap();
    attach(&root, "task_0001", "two.txt", TWO).await.unwrap();
    ...
    assert_eq!(
        get(&root, "task_0001", false).await.unwrap()[0].attachments(),
        [an("one.txt"), an("two.txt")]
    );
}

#[tokio::test]
async fn an_attachment_name_already_taken_is_refused() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "t", "", "").await.unwrap();
    attach(&root, "task_0001", "one.txt", ONE).await.unwrap();
    ...
    assert!(matches!(
        attach(&root, "task_0001", "one.txt", TWO)
            .await
            .unwrap_err(),
...
#[tokio::test]
async fn an_attachment_to_a_missing_task_is_not_found() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "real", "", "").await.unwrap();
    assert!(matches!(
        attach(&root, "ghost/task_0001", "hello.txt", HELLO)
            .await
            .unwrap_err(),
...
#[tokio::test]
async fn an_update_rewrites_the_markdown_inside_a_task_directory() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "t", "", "").await.unwrap();
    attach(&root, "task_0001", "one.txt", ONE).await.unwrap();

    let updated = advance(&root, "task_0001", "completed", Some("all done"))
        .await
        .unwrap();
...
#[tokio::test]
async fn a_move_carries_a_task_directory_and_its_attachments() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "t", "shopping", "").await.unwrap();
    attach(&root, "shopping/task_0001", "one.txt", ONE)
        .await
        .unwrap();
    ...
    assert_eq!(
        get(&root, "dev/task_0001", false).await.unwrap()[0].attachments(),
        [an("one.txt")]
    );
}

#[tokio::test]
async fn a_created_task_never_reuses_the_number_of_a_task_directory() {
    let dir = fixture_dir();
    let root = root(&dir);
...
#[tokio::test]
async fn a_get_returns_a_task_directory_and_never_its_attachments() {
    let dir = fixture_dir();
    let root = root(&dir);
    create(&root, "t", "", "").await.unwrap();
    attach(&root, "task_0001", "sidecar.md", ONE).await.unwrap();

    assert_eq!(
        paths(&get(&root, "", false).await.unwrap()),
        vec!["task_0001"]
    );
    assert!(get(&root, "task_0001", false).await.unwrap().len() == 1);
}
...
async fn a_search_matches_the_markdown_inside_a_task_directory() {
    ...
    attach(&root, "dev/task_0001", "one.txt", ONE)
        .await
        .unwrap();
...
async fn a_search_never_returns_an_attachment_as_a_task() {
    ...
    attach(&root, "dev/task_0001", "sidecar.md", NEEDLE)
        .await
        .unwrap();
