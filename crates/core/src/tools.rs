...
const D_WRITE: &str = "Write a note, overwriting it. Creates parent directories. Never use for logging or timestamped entries — those must go through LogNote. Paths under .logs/ and .tasks/ are refused: log entries are write-once, and a task is created with CreateTask and changed with UpdateTask/MoveTask.";
